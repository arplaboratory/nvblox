...
def generate_launch_description():
    ...
    group_action = GroupAction([
        ...
        SetParameter(name='global_frame',
                     value=LaunchConfiguration('global_frame', default='world')),
        SetParameter(name='pose_frame',
                     value=LaunchConfiguration('pose_frame', default='cam1_infra1_optical_frame')),

	SetParameter(name='use_tf_transforms',
                     value=LaunchConfiguration('use_tf_transforms', default='True')),
        SetParameter(name='use_topic_transforms',
                     value=LaunchConfiguration('use_topic_transforms', default='False')),
        SetParameter(name='use_lidar',
                     value=LaunchConfiguration('use_lidar', default='False')),
        SetParameter(name='slice_visualization_attachment_frame_id',
                     value=LaunchConfiguration('slice_visualization_attachment_frame_id', default='race15/base_link')),
        SetParameter(name='use_depth',
                     value=LaunchConfiguration('use_depth', default='True')),
        SetParameter(name='map_clearing_frame_id',
                     value=LaunchConfiguration('map_clearing_frame_id', default='race15/base_link')),

        # Remappings for realsense data
        SetRemap(src=['depth/image'],
                 dst=['/stereo/img_depth'],
                 condition=setup_for_realsense),
        SetRemap(src=['depth/camera_info'],
                 dst=['/race15/cam1/infra1/camera_info'],
                 condition=setup_for_realsense),
